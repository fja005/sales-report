...
TRANSLATIONS = {
    "es": {
        ...
        "hero_subtitle": "Sube un Excel o CSV y obtén métricas, gráficos, utilidad e insights automáticos por negocio.",
        ...
        "required_optional": "Columna opcional para insights avanzados:",
    ...
    "en": {
        ...
        "hero_subtitle": "Upload an Excel or CSV file and get metrics, charts, profitability and automatic insights by business.",
        ...
        "required_optional": "Optional column for advanced insights:",
    ...
    "id": {
        ...
        "hero_subtitle": "Unggah file Excel atau CSV dan dapatkan metrik, grafik, profitabilitas, serta insight otomatis per bisnis.",
        ...
        "required_optional": "Kolom opsional untuk insight lanjutan:",
        ...
        "actions": "Tindakan yang disarankan",
...
def validar_columnas(df):
    ...
    return columnas_esperadas - columnas_archivo
...
def normalizar_dataframe(df):
    ...
    tiene_costos = "costo_unitario" in df.columns
    if tiene_costos:
    ...
    if tiene_costos:
        ...
        df["margen_pct"] = df.apply(
            lambda row: (row["utilidad"] / row["ingreso"] * 100) if row["ingreso"] > 0 else 0,
            axis=1
        )
    else:
    ...
    subset_cols = ["fecha", "producto", "categoria", "cantidad", "precio"]
    if tiene_costos:
        subset_cols.append("costo_unitario")

    df = df.drop_duplicates(subset=subset_cols)
    return df, tiene_costos
...
def guardar_ventas_en_db(df, negocio):
    ...
    for _, row in df.iterrows():
        costo_unitario = None if pd.isna(row["costo_unitario"]) else float(row["costo_unitario"])
...
def obtener_dataframe_db(negocio):
    ...
    if not ventas:
        return pd.DataFrame(columns=[
            ...
            "costo_unitario", "ingreso", "costo", "utilidad", "margen_pct"
        ])
    ...
    tiene_costos = "costo_unitario" in df.columns and df["costo_unitario"].notna().any()
    if tiene_costos:
        df["costo_unitario"] = pd.to_numeric(df["costo_unitario"], errors="coerce").fillna(0)
        df["costo"] = df["cantidad"] * df["costo_unitario"]
        df["utilidad"] = df["ingreso"] - df["costo"]
...
def generar_grafico_ventas_por_dia(df, filename):
    ventas_dia = df.groupby("fecha")["ingreso"].sum().sort_index()
...
def generar_grafico_ventas_por_categoria(df, filename):
    ventas_categoria = df.groupby("categoria")["ingreso"].sum().sort_values(ascending=False)
    plt.figure(figsize=(10, 5))
...
def construir_insights(df, lang, tiene_costos):
    insights = []
    acciones = []

    if df.empty:
        return insights, acciones

    total_ingresos = df["ingreso"].sum()
    ticket_promedio = df["ingreso"].mean()

    producto_top = df.groupby("producto")["ingreso"].sum().sort_values(ascending=False)
    categoria_top = df.groupby("categoria")["ingreso"].sum().sort_values(ascending=False)

    if not producto_top.empty:
        prod = producto_top.index[0]
        share = (producto_top.iloc[0] / total_ingresos * 100) if total_ingresos > 0 else 0
        insights.append(f'{tr(lang, "top_products")}: {prod} ({share:.1f}%)')
        if share > 35:
            acciones.append(f'{prod}: revisar precio o asegurar stock, porque concentra gran parte de las ventas.')

    if not categoria_top.empty:
        cat = categoria_top.index[0]
        share_cat = (categoria_top.iloc[0] / total_ingresos * 100) if total_ingresos > 0 else 0
        insights.append(f'{tr(lang, "sales_by_category")}: {cat} ({share_cat:.1f}%)')
        if share_cat > 50:
            acciones.append("Diversificar ventas entre categorías para reducir dependencia.")

    if ticket_promedio < 10:
        insights.append(f'{tr(lang, "avg_ticket")}: {ticket_promedio:.2f}')
        acciones.append("Probar combos, upselling o alzas moderadas de precio para subir el ticket promedio.")

    ventas_dia = df.groupby("fecha")["ingreso"].sum().sort_index()
    if len(ventas_dia) >= 6:
        primeras = ventas_dia.head(len(ventas_dia)//2).mean()
        ultimas = ventas_dia.tail(len(ventas_dia)//2).mean()
        if ultimas < primeras * 0.9:
            insights.append("Tendencia reciente: caída de ventas.")
            acciones.append("Revisar promociones, mix de productos o días de menor rendimiento.")
        elif ultimas > primeras * 1.1:
            insights.append("Tendencia reciente: crecimiento de ventas.")

    if tiene_costos and df["ingreso"].sum() > 0:
        margen_total = (df["utilidad"].sum() / df["ingreso"].sum() * 100)
        if margen_total < 25:
            insights.append(f'{tr(lang, "margin")}: {margen_total:.1f}%')
            acciones.append("Revisar costos unitarios y precios en productos de bajo margen.")

        utilidad_producto = df.groupby("producto")["utilidad"].sum().sort_values()
        if not utilidad_producto.empty and utilidad_producto.iloc[0] < 0:
            acciones.append(f'Revisar {utilidad_producto.index[0]}: está destruyendo utilidad.')

    if not acciones:
        acciones.append("Mantener seguimiento semanal del producto estrella y la categoría dominante.")
...
def generar_excel_reporte(df):
    ...
    resumen = pd.DataFrame({
        "metrica": ["Ingresos", "Ticket Promedio", "Cantidad de registros"],
        "valor": [round(df["ingreso"].sum(), 2), round(df["ingreso"].mean(), 2), len(df)]
    })
    ...
    if "utilidad" in df.columns and df["utilidad"].sum() != 0:
        ...
        top_profit_products = pd.DataFrame(columns=["producto", "utilidad"])
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        profit_by_category.to_excel(writer, index=False, sheet_name="Utilidad por categoria")
        top_profit_products.to_excel(writer, index=False, sheet_name="Top utilidad")
    ...
    try:
        ...
        if faltantes:
            return render_template(
                ...
                lang=lang,
                t=TRANSLATIONS[lang],
                error=f'{tr(lang, "missing_columns_prefix")} ' + ", ".join(sorted(faltantes)) + f'. {tr(lang, "missing_columns_suffix")}'
            )

        df, tiene_costos = normalizar_dataframe(df)

        if df.empty:
            return render_template("index.html", lang=lang, t=TRANSLATIONS[lang], error=tr(lang, "empty_after_clean"))

        nuevas, duplicadas = guardar_ventas_en_db(df, negocio)

        ingreso_total = round(df["ingreso"].sum(), 2)
        costo_total = round(df["costo"].sum(), 2)
        utilidad_total = round(df["utilidad"].sum(), 2)
        margen_total = round((utilidad_total / ingreso_total * 100), 2) if ingreso_total > 0 and tiene_costos else None
        ...
        insights, acciones = construir_insights(df, lang, tiene_costos)
        ...
        session["last_result"] = {
            ...
            "ventas_por_categoria": {str(k): float(v) for k, v in ventas_por_categoria.items()},
            "top_productos": {str(k): int(to_python_type(v)) for k, v in top_productos.items()},
            "chart_dia": str(chart_dia),
            "chart_categoria": str(chart_categoria),
            "nuevas": int(to_python_type(nuevas)),
            "duplicadas": int(to_python_type(duplicadas)),
            "ingreso_total": float(ingreso_total),
            "costo_total": float(costo_total),
            "utilidad_total": float(utilidad_total),
            "margen_total": float(margen_total) if margen_total is not None else None,
            "tiene_costos": bool(tiene_costos),
            "top_profit_products": {str(k): float(v) for k, v in top_profit_products.items()},
            "profit_by_category": {str(k): float(v) for k, v in profit_by_category.items()},
            "insights": [str(x) for x in insights],
            "acciones": [str(x) for x in acciones]
        }

        return redirect(url_for("resultado", lang=lang))
...
def resultado():
    ...
    if not data:
        return render_template("index.html", lang=lang, t=TRANSLATIONS[lang], error=tr(lang, "no_result_available"))
    return render_template("result.html", lang=lang, t=TRANSLATIONS[lang], **data)
...
def dashboard():
    ...
    if not negocio:
        return render_template("dashboard.html", lang=lang, t=TRANSLATIONS[lang], pedir_negocio=True, sin_datos=False)
    ...
    if tiene_costos:
        top_profit_products = (
            df.groupby("producto")["utilidad"]
            .sum().sort_values(ascending=False).head(10).round(2).to_dict()
        )
        profit_by_category = (
            df.groupby("categoria")["utilidad"]
            .sum().sort_values(ascending=False).round(2).to_dict()
...
def descargar_dashboard():
    ...
    return send_file(excel, download_name=f"reporte_ventas_{negocio}.xlsx", as_attachment=True)
...
def reiniciar_datos():
    ...
    if not negocio:
        return render_template("index.html", lang=lang, t=TRANSLATIONS[lang], error=tr(lang, "must_indicate_business"))
